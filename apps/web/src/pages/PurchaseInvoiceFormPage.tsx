...
import { PurchaseInvoice, PurchaseInvoiceLine, Supplier } from '../types/invoices';
...
export const PurchaseInvoiceFormPage: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            if (id) {
                const inv = await api<PurchaseInvoice>(`/purchase-invoices/${id}`);
                setInvoice(inv);
            }
        } catch (err) {
    ...
    const updateLine = (index: number, updates: Partial<PurchaseInvoiceLine>) => {
    ...
    const handleSave = async (post: boolean = false) => {
        ...
        try {
            ...
            if (id) {
                // Update logic (if implemented in API)
                result = await api<PurchaseInvoice>(`/purchase-invoices/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
            } else {
                result = await api<PurchaseInvoice>('/purchase-invoices', { method: 'POST', body: JSON.stringify(payload) });
            }

            if (post) {
                await api(`/purchase-invoices/${result.id}/post`, { method: 'POST' });
            }
    ...
    const getLineIndex = (line: PurchaseInvoiceLine) => (invoice.lines || []).findIndex((candidate) => candidate.id === line.id);

    const columns: Column<PurchaseInvoiceLine>[] = [
        {
                ...
                return (
                    <Select
                        ...
                        onChange={(e) => {
                            ...
                            updateLine(index, { product_id: pid, unit_price: prod?.cost || 0 });
                        }}
                    >
    ...
    return (
        <div className="purchase-invoice-form">
            <PageHeader
                title={id ? t('invoices.actions.editPurchase') : t('invoices.actions.newPurchase')}
                ...
                <Card className="md:col-span-3">
                    <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
                        <Select 
                            ...
                            onChange={(e) => setInvoice({...invoice, supplier_id: Number(e.target.value)})}
                        >
                            <option value="">{t('common.select')}</option>
                            {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </Select>
                        <Input 
                            label={t('suppliers.payableAccount')} 
                            value={suppliers.find(s => s.id === invoice.supplier_id)?.payable_account_code || ''}
                            readOnly
                            disabled
                        />
                        <Input 
